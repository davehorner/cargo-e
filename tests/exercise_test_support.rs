...
use common::test_utils::{
    create_testgen_ex_project, create_testgen_bin_project, create_testgen_ext_project, count_samples,
};
...
fn exercise_test_support_functions() {
    ...
    let ex_project = create_testgen_ex_project(ex_name)
        .expect("Failed to create testgen_ex project");
    ...
    let example_path = ex_project.path().join("examples").join(format!("{}.rs", ex_name));
    assert!(
        example_path.exists(),
    ...
    let bin_project = create_testgen_bin_project(bin_name)
        .expect("Failed to create testgen_bin project");
    
    // Verify that the binary file exists.
    let bin_path = bin_project.path().join("src").join("bin").join(format!("{}.rs", bin_name));
    assert!(
        bin_path.exists(),
    ...
    let ext_project = create_testgen_ext_project(ext_name)
        .expect("Failed to create testgen_ext project");
