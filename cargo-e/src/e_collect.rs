        ...
        for (_prefix, manifest_path, _extended) in manifest_infos {
            ...
            pool.execute(move || {
                ...
                if bins.is_empty() && examples.is_empty() {
                    println!("No runnable binaries or examples found.");
                    return;
                }

                // Continue processing the targets...
                println!("{:?}Binaries: {:?}", &manifest_clone, bins);
                println!("{:?}Examples: {:?}", &manifest_clone, examples);
                println!("{:?}Benches: {:?}", &manifest_clone, benches);
                println!("{:?}Tests: {:?}", &manifest_clone, tests);
                println!(
                    "{} bins, {} examples, {} benches, {} tests",
                    bins.len(),
                    examples.len(),
                    benches.len(),
                    tests.len()
