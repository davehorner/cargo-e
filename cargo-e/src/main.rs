...
pub fn main() -> anyhow::Result<()> {
    ...
    if let (Some(stdout_path), Some(stderr_path)) = (stdout_path, stderr_path) {
        ...
        use cargo_e::e_diagnostics_dispatchers::{create_stdout_dispatcher, create_stderr_dispatcher};
        ...
        let manifest_path = std::env::current_dir()?.join("Cargo.toml").to_string_lossy().to_string();
        let cargo_stats = Arc::new(Mutex::new(CargoStats::default()));
        let _stdout_dispatcher = create_stdout_dispatcher();
        let _stderr_dispatcher = create_stderr_dispatcher(diagnostics.clone(), manifest_path.clone());
